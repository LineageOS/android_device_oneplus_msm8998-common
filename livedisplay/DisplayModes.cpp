...
Return<void> DisplayModes::getCurrentDisplayMode(getCurrentDisplayMode_cb resultCb) {
    ...
    for (const auto& entry : kModeMap) {
        ...
        if (!modeFile.fail()) {
            ...
            LOG(ERROR) << "Failed reading mode file " << std::string(entry.second.value) << ". Result: " << modeFile.fail();
        }
    }
...
Return<bool> DisplayModes::setDisplayMode(int32_t modeID, bool makeDefault) {
    ...
    for (const auto& entry : kModeMap) {
        ...
        if (!modeFile.fail()) {
            ...
            LOG(ERROR) << "Failed writing mode file " << std::string(entry.second.value) << ". Result: " << modeFile.fail();
        }
    }
    ...
    if (modeID != 0) {
        ...
        modeFile << 1;
        if (modeFile.fail()) {
            LOG(ERROR) << "Failed writing mode file " << std::string(iter->second.value) << ". Result: " << modeFile.fail();
            return false;
        }
    }
